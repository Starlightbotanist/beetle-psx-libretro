...
#ifdef BEETLE_VULKAN_PIPELINE_DIAGNOSTICS
      LOGI("[Vulkan shader precompilation pool] phase=%s actual_workers=%u helper_threads=%u jobs=%u cache_mode=shared batch_size=%u\n",
            cache_probe ? "cache-probe" : "compile", workers,
            workers - 1, pending, VulkanPrecompileBatchSize);
#endif
...
#ifdef BEETLE_VULKAN_PIPELINE_DIAGNOSTICS
      for (i = 0; i < workers; i++)
         LOGI("[Vulkan shader precompilation worker] phase=%s index=%u jobs=%u graphics_jobs=%u compute_jobs=%u driver_calls=%u pipeline_elapsed_us=%lld wall_elapsed_us=%lld\n",
               cache_probe ? "cache-probe" : "compile", i,
               worker[i].jobs, worker[i].graphics_jobs,
               worker[i].compute_jobs, worker[i].driver_calls,
...
/* #undef VULKAN_DEBUG */
   ...
   /* Persistent cache implementation stays in this translation unit because
    * it operates on private Device state, but lives separately so its file,
    * lock and compatibility policy can be reviewed and tested as one unit. */
