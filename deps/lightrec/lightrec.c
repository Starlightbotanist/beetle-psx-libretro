 ...
 * instruction is passed directly because execution may have crossed a block
 * boundary by the time the wrapper runs. Source operands were saved before
 * execution so destination/source aliases retain the consumed values. */
